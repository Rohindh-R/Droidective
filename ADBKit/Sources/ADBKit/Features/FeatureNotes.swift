...
extension FeatureRegistry {
    ...
    private static let notes: [String: String] = [
        ...
        "network-speed": "Live download/upload throughput, sampled from `/proc/net/dev` byte counters once a second. Shows the device-wide total, a per-interface breakdown (wlan0, cellular…), and session totals; record and export as JSON + CSV. Device-wide only — per-app traffic needs root on modern Android.",

...
extension FeatureRegistry {
    ...
    private static let notes: [String: String] = [
        ...
        "file-explorer": "Browse the device's shared storage. Copy/cut/paste, delete, create folders, and pull files to your Mac. On a rooted device, flip Root on to browse the whole filesystem from / via su (pulls stage through /data/local/tmp).",
        "apps": "Every installed app (user + system) with search. Select one for its info, live permission toggles, APK pull, and lifecycle actions — disable/enable, or uninstall-for-user/restore (`pm disable-user` / `pm uninstall --user 0` / `cmd package install-existing`). All per-user and reversible, so it doubles as a debloater; removing core system apps can break the UI, so stick to ones you recognise.",
